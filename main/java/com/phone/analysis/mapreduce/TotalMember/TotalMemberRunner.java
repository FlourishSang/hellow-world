...
public class TotalMemberRunner implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        //设置map相关参数
        job.setMapperClass(TotalMemberMapper.class);
        ...
        //设置reduce相关参数
        //设置reduce端的输出格式类
        job.setReducerClass(TotalMemberReducer.class);
    ...
    //该方法是计算新增总用户的（重点）
    private int calculateTotalMember(Job job) {
        ...
        try {
            ...
            if (nowday_DimensionID > 0) {
                ...
                ps = conn.prepareStatement(conf.get("other_new_total_browser_member_now_sql"));//通过name获取sql查询语句
            ...
            if (yesterday_DimensionID > 0) {
                ...
                while (rs.next()) {
                    ...
                    if (map.containsKey(key)) {
                        totalMembers += map.get(key);
                    }
                    map.put(key, totalMembers);
                }
            }
...
//                    Date date3 = new Date(date2);
    ...
    private void handleInputOutput(Job job) {
        ...
        try {
            ...
            /*
             ** 此处是hdfs输入路径：需要去拼接，通过设置的参数，-d 2017-05-28这两个参数，获取到第二个参数，
