...
import com.phone.analysis.mapreduce.HourlyActiveUser.HourlyActiveUserMapper;
import com.phone.analysis.mapreduce.HourlyActiveUser.HourlyActiveUserReducer;
 ...
 **/
public class LocationInfoRunner implements Tool {

    private static final Logger logger = Logger.getLogger(LocationInfoRunner.class);
    ...
    public static void main(String[] args){
        try {
            ...
            ToolRunner.run(new Configuration(),new LocationInfoRunner(),args);
        } catch (Exception e) {
    ...
    public int run(String[] args) throws Exception {
        ...
        job.setJarByClass(LocationInfoRunner.class);

        //设置map相关参数
        job.setMapperClass(LocationInfoMapper.class);
        job.setMapOutputKeyClass(StatsLocationDimension.class);
        job.setMapOutputValueClass(LocationMapOutputValue.class);
        //设置本地提交集群运行，要使用job.setJar()将打包好的jar包路径写入
        //job.setJar("D:\\javaProject\\GP1814Log_Analystic\\target\\analystic-1.0.jar");

        //设置reduce相关参数
        //设置reduce端的输出格式类
        job.setReducerClass(LocationInfoReducer.class);
        job.setOutputKeyClass(StatsLocationDimension.class);
        job.setOutputValueClass(LocationReduceOutputValue.class);
